    ...
    while retries < max_retries:
        ...
        try:
            ...
            try:
                ...
                await page.wait_for_selector(f"xpath={player_name_xpath}", timeout=10000)
                locator = page.locator(f"xpath={player_name_xpath}")
                player_name = await locator.inner_text()
...
async def scrape_tournament_data(tournament_url, age_group, draw_size, sort, tournament_level):
    ...
    col_widths = [20, 110, 110, 90, 40, 40, 50, 50, 60, 60, 40]  # first value = "No" column width
...
# --- Main Runner with Cleanup ---
async def main():
    tournament_link = input("Enter the tournament link: ")
    age_options = await age_groups_level(tournament_link)
    if len(age_options[2]) == 1:
        age_group = age_options[2][0]
    else:
        for i in age_options[2]:
            print(str(age_options[2].index(i) + 1) + ". " + i)
        age_group = age_options[2][int(input("Enter the number for your selected age group: ")) - 1]
    sort = await scrape_draw_size(tournament_link.replace("overview", "events"), age_group)
    await scrape_tournament_data(tournament_link.lower(), age_group, sort[0], sort[1], age_options[0])
