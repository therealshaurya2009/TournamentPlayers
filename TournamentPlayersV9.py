...
async def age_groups_level(tournament_link):
    ...
    try:
        ...
        await page.wait_for_selector(f"xpath={level_xpath}", timeout=wait)
...
async def scrape_recruiting(name, location, page):
    ...
    while (player_rating == "Unknown" or player_year == "Unknown" or player_utr == "0.xx") and retries < max_retries:
        ...
        try:
            await page.wait_for_selector(f"xpath={player_rating_xpath}", timeout=wait)
            player_rating = await page.locator(f"xpath={player_rating_xpath}").get_attribute("src")
        except:
            player_rating = "https://www.tennisrecruiting.net/img/record.gif"

        try:
            await page.wait_for_selector("text=.xx", timeout=wait)
            player_utr = await page.locator("text=.xx").nth(0).inner_text()
        except:
            player_utr = "0.xx"

        try:
            await page.wait_for_selector(f"xpath={player_year_xpath}", timeout=wait)
    ...
    return [player_rating, player_utr, player_year]
...
async def scrape_tournament_data(tournament_url, age_group, draw_size, sort, tournament_level):
    ...
    table_data = [["No", "Name", "Location", "District", "Seed", "WTN", "Points", "Rank", "Recruit", "Grade", "UTR"]]
    ...
    for i in range(len(player_names)):
        ...
        row = [
            ...
            player_districts[i] if i < len(player_districts) else "",
            ...
            player_rankings[i] if i < len(player_rankings) else "",
            player_recruiting[i] if i < len(player_recruiting) else "",
            player_year[i] if i < len(player_year) else "",
            player_utr[i] if i < len(player_utr) else ""
            ]
        table_data.append(row)

    # Example column widths (adjust as needed)
    col_widths = [20, 110, 100, 100, 40, 40, 40, 50, 60, 65, 35]  # first value = "No" column width
