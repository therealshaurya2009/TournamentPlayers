...
def main():
    ...
    # Initialize session state to store age groups
    ...
    if st.button("Find age groups:"):
        age_options = asyncio.run(age_groups_level(tournament_link))
        st.session_state.age_groups_final = age_options[2]  # store in session state

    # Only show dropdown if we have age groups
